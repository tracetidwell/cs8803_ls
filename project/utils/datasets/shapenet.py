...
class ShapeNet(ProjectInMemoryDataset):
    ...
    def __init__(self,
        ...
        super(ShapeNet, self).__init__(root, transform, pre_transform,
                                       pre_filter, raw_name, processed_name)
    ...
    def process_subset(self, data_path, label_path):
        y_offset = 0
        data_list = []
        cat_ys = []
        for cat_idx, cat in enumerate(self.categories):
            idx = self.category_ids[cat]
            #point_paths = sorted(glob.glob(osp.join(data_path, idx, '*.pts')))
            point_paths = sorted([osp.join(data_path, idx, point_path) for point_path in self.train_dict[idx]])
            y_paths = sorted([osp.join(label_path, idx, point_path[:-4]+'.seg') for point_path in self.train_dict[idx]])
            #y_paths = sorted(glob.glob(osp.join(label_path, idx, '*.seg')))
    ...
    def process(self):
        if self.train_dict is not None:
            train_data_list, y_mask = self.process_subset(*self.raw_paths[0:2])
        else:
